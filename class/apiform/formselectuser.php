 ...
 *
 * @copyright       The API Project (http://api.org)
 ...
 * User Select field
 */
class APIFormSelectUser extends APIFormElementTray
{
    /**
     * Constructor
     *
    ...
    public function __construct($caption, $name, $includeAnonymous = false, $value = null, $size = 1, $multiple = false)
    {
        ...
        $select_element = new APIFormSelect('', $name, $value, $size, $multiple);
        if ($includeAnonymous) {
            $select_element->addOption(0, $GLOBALS['apiConfig']['anonymous']);
        }
        ...
        if (empty($queryCache)) {
            ...
            $queryCache = APICache::read($cachekey);
            if ($queryCache === false) {
                ...
                asort($queryCache);
                APICache::write($cachekey, $queryCache, $cachettl); // won't do anything different if write fails
            }
        }
        ...
        api_loadLanguage('findusers');
        $js_addusers = "
            function addusers(opts)
            {
                var num = opts.substring(0, opts.indexOf(':'));
                opts = opts.substring(opts.indexOf(':')+1, opts.length);
                var sel = apiGetElementById('" . $name . "');
        ...
        $token       = $GLOBALS['apiSecurity']->createToken();
        $action_tray = new APIFormElementTray('', '');
        $removeUsers = new APIFormButton('', 'rmvusr_' . $name, _MA_USER_REMOVE, 'button');
        $removeUsers->setExtra(' onclick="var sel = apiGetElementById(\'' . $name . '\');for (var i = sel.options.length-1; i >= 0; i--) {if (!sel.options[i].selected) {sel.options[i] = null;}}; return false;" ');
        $action_tray->addElement($removeUsers);

        $searchUsers = new APIFormButton('', 'srchusr_' . $name, _MA_USER_MORE, 'button');
        $searchUsers->setExtra(' onclick="openWithSelfMain(\'' . API_URL . '/include/findusers.php?target=' . $name . '&amp;multiple=' . $multiple . '&amp;token=' . $token . '\', \'userselect\', 800, 600, null); return false;" ');
