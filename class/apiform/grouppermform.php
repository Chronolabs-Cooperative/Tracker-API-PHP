 ...
 */
class APIGroupPermForm extends APIForm
{
    /**
     * Module ID
     *
    ...
    {
        parent::__construct($title, 'groupperm_form', API_URL . '/modules/system/admin/groupperm.php', 'post');
        $this->_modid    = (int)$modid;
        $this->_permName = $permname;
        $this->_permDesc = $permdesc;
        $this->addElement(new APIFormHidden('modid', $this->_modid));
        $this->addElement(new APIFormHiddenToken($permname));
        if ($url != '') {
            $this->addElement(new APIFormHidden('redirect_url', $url));
        }
    ...
    public function render()
    {
        ...
        /* @var  $gperm_handler APIGroupPermHandler */
        $gperm_handler  = api_getHandler('groupperm');
        /* @var $member_handler APIMemberHandler */
        $member_handler = api_getHandler('member');
        $glist          = $member_handler->getGroupList();
        foreach (array_keys($glist) as $i) {
            if ($i == API_GROUP_ANONYMOUS && !$this->_showAnonymous) {
                continue;
            }
            // get selected item id(s) for each group
            $selected = $gperm_handler->getItemIds($this->_permName, $i, $this->_modid);
            $ele      = new APIGroupFormCheckBox($glist[$i], 'perms[' . $this->_permName . ']', $i, $selected);
            $ele->setOptionTree($this->_itemTree);
            $this->addElement($ele);
            unset($ele);
        }
        $tray = new APIFormElementTray('');
        $tray->addElement(new APIFormButton('', 'submit', _SUBMIT, 'submit'));
        $tray->addElement(new APIFormButton('', 'reset', _CANCEL, 'reset'));
 ...
 */
class APIGroupFormCheckBox extends APIFormElement
{
    /**
    ...
    public function render()
    {
        ...
        $ret .= _ALL . " <input id=\"" . $checkallbtn_id . "\" type=\"checkbox\" value=\"\" onclick=\"var optionids = new Array(" . $option_ids_str . "); apiCheckAllElements(optionids, '" . $checkallbtn_id . "');\" />";
        $ret .= '</td></tr></table>';
    ...
    public function _renderOptionTree(&$tree, $option, $prefix, $parentIds = array())
    {
        ...
        foreach ($parentIds as $pid) {
            ...
            $tree .= "var ele = apiGetElementById('" . $parent_ele . "'); if(ele.checked != true) {ele.checked = this.checked;}";
        }
        ...
        foreach ($option['allchild'] as $cid) {
            ...
            $tree .= "var ele = apiGetElementById('" . $child_ele . "'); if(this.checked != true) {ele.checked = false;}";
        }
        $tree .= '" value="1"';
