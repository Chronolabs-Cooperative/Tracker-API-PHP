...
if (!function_exists("setCallBackURI")) {
	...
	{
		list($when) = $GLOBALS['trackerDB']->fetchRow($GLOBALS['trackerDB']->queryF("SELECT `when` from `callbacks` ORDER BY `when` DESC LIMIT 1"));
		if ($when<time())
			$when = $time();
			$when = $when + mt_rand(3, 14);
		return $GLOBALS['trackerDB']->queryF("INSERT INTO `callbacks` (`when`, `uri`, `timeout`, `connection`, `data`, `queries`) VALUES(\"$when\", \"$uri\", \"$timeout\", \"$connectout\", \"" . mysql_escape_string(json_encode($data)) . "\",\"" . mysql_escape_string(json_encode($queries)) . "\")");
	}
}
...
if (!function_exists("getAPIDataArray")) {
	...
	function getAPIDataArray($mode = '', $clause = '', $state = '', $name = '', $output = '', $version = 'v2')
	{
		...
		{
			case "torrents":
				$result = $GLOBALS['trackerDB']->queryF("SELECT * from `torrents` ORDER BY `last_action` DESC");
				...
				break;
			case "peers":
				list($torrentid) = $GLOBALS['trackerDB']->fetchRow($GLOBALS['trackerDB']->queryF("SELECT `id` from `torrents` WHERE `info_hash` = '$clause' ORDER BY RAND() DESC LIMIT 1"));
				$result = $GLOBALS['trackerDB']->queryF("SELECT * from `peers` WHERE torrentid = '$torrentid' AND `seeder` = 'no' ORDER BY RAND()");
				while($row = $GLOBALS['trackerDB']->fetchArray($result))
				{
			...
			case "seeds":
				list($torrentid) = $GLOBALS['trackerDB']->fetchRow($GLOBALS['trackerDB']->queryF("SELECT `id` from `torrents` WHERE `info_hash` = '$clause' ORDER BY RAND() DESC LIMIT 1"));
				$result = $GLOBALS['trackerDB']->queryF("SELECT * from `peers` WHERE torrentid = '$torrentid' AND `seeder` = 'yes' ORDER BY RAND()");
				while($row = $GLOBALS['trackerDB']->fetchArray($result))
				{
					$return[$hash = md5($row['id'].API_URL.$row['apiid'].$row['peerid'])] = $row;
					unset($return[$hash]['torrentid']);
			...
			case "files":
				list($torrentid) = $GLOBALS['trackerDB']->fetchRow($GLOBALS['trackerDB']->queryF("SELECT `id` from `torrents` WHERE `info_hash` = '$clause' ORDER BY RAND() DESC LIMIT 1"));
				$result = $GLOBALS['trackerDB']->queryF("SELECT * from `files` WHERE torrentid = '$torrentid'");
				while($row = $GLOBALS['trackerDB']->fetchArray($result))
				{
			...
			case "trackers":
				list($torrentid) = $GLOBALS['trackerDB']->fetchRow($GLOBALS['trackerDB']->queryF("SELECT `id` from `torrents` WHERE `info_hash` = '$clause' ORDER BY RAND() DESC LIMIT 1"));
				$result = $GLOBALS['trackerDB']->queryF("SELECT * from `trackers_to_torrents` WHERE torrentid = '$torrentid' AND `seeder` = 'yes' ORDER BY RAND()");
				$trackerids = array();
				while($row = $GLOBALS['trackerDB']->fetchArray($result))
				{
				...
				$result = $GLOBALS['trackerDB']->queryF("SELECT * from `trackers` WHERE `id` IN (".implode(", ", $trackerids).")");
				while($row = $GLOBALS['trackerDB']->fetchArray($result))
				{
			...
			case "network":
				list($torrentid) = $GLOBALS['trackerDB']->fetchRow($GLOBALS['trackerDB']->queryF("SELECT `id` from `torrents` WHERE `info_hash` = '$clause' ORDER BY RAND() DESC LIMIT 1"));
				$result = $GLOBALS['trackerDB']->queryF("SELECT * from `networking_to_torrents` WHERE torrentid = '$torrentid' AND `seeder` = 'yes' ORDER BY RAND()");
				$ipids = array();
				while($row = $GLOBALS['trackerDB']->fetchArray($result))
				{
				...
				$result = $GLOBALS['trackerDB']->queryF("SELECT * from `trackers` WHERE `id` IN ('".implode("', '", $ipids)."')");
				while($row = $GLOBALS['trackerDB']->fetchArray($result))
				{
...
if (!function_exists("getRandomInfoHash")) {
	...
	function getRandomInfoHash( ) {
		...
		{
			$sql = "SELECT * FROM `torrents` ORDER BY RAND() LIMIT 1";
			if ($GLOBALS['trackerDB']->getRowsNum($results = $GLOBALS['trackerDB']->queryF(sprintf($sql, mysql_escape_string($infohash))))==1)
			{
				$torrent = $GLOBALS['trackerDB']->fetchArray($results);
			}
			$i++;
		}
...
if (!function_exists("getNetworkingArray")) {
	...
	function getNetworkingArray( $ipid = '' ) {
		...
		$sql = "SELECT * FROM `networking` WHERE `id` LIKE '%s'";
		if ($GLOBALS['trackerDB']->getRowsNum($results = $GLOBALS['trackerDB']->queryF(sprintf($sql, mysql_escape_string($infohash))))==1)
		{
			$network = $GLOBALS['trackerDB']->fetchArray($results);
			return $network;
		} else {
			return array();
		}
...
if (!function_exists("getTorrentIdentity")) {
	...
	function getTorrentIdentity( $infohash = '' ) {
		...
		$sql = "SELECT * FROM `torrents` WHERE `info_hash` LIKE '%s'";
		if ($GLOBALS['trackerDB']->getRowsNum($results = $GLOBALS['trackerDB']->queryF(sprintf($sql, mysql_escape_string($infohash))))==1)
		{
			$torrent = $GLOBALS['trackerDB']->fetchArray($results);
			return $torrent['id'];
		} else {
			$sql = "INSERT INTO `torrents` (`info_hash`, `added`, `owner_ipid`) VALUES ('%s', unix_timestamp(), '%s')";
			if ($GLOBALS['trackerDB']->queryF(sprintf($sql, mysql_escape_string($infohash), mysql_escape_string($GLOBALS['ipid']))))
			{
				$torrentid = $GLOBALS['trackerDB']->getInsertID();
				$sql = "INSERT INTO `networking_to_torrents` (`ipid`, `torrentid`, `start`) VALUES('%s','%s','%s')";
				$GLOBALS['trackerDB']->queryF(sprintf($sql, $GLOBALS['ipid'],$torrentid,time()));
				return $torrentid;
			}
...
if (!function_exists("getAPIIdentity")) {
	 ...
	 * @param string $root
	 */
	function getAPIIdentity( $url,  $version, $callback, $polinating = true, $root = "http://tracker.labs.coop" ) {
		
		$sql = "SELECT * FROM `apis` WHERE `api-url` LIKE '%s'";
		if ($GLOBALS['trackerDB']->getRowsNum($results = $GLOBALS['trackerDB']->queryF(sprintf($sql, mysql_escape_string($url))))==1)
		{
			...
			if (strpos($url, 'localhost')>0 || $url = $root)
				$polinating = false;
			$sql = "INSERT INTO `peers` (`peer-id`, `api-url`, `version`, `callback`, `created`) VALUES ('%s', '%s', '%s', '%s', '%s')";
			$apiid = md5($url.$version.$callback.$polinating.$root.microtime(true));
			if ($GLOBALS['trackerDB']->queryF(sprintf($sql, mysql_escape_string($apiid), mysql_escape_string($url), mysql_escape_string($version), mysql_escape_string($callback), time())))
			{
				if ($polinating==true)
...
if (!function_exists("getCountryID")) {
	...
	function getCountryID($country = '')
	{
		...
		if (empty($countries))
			$countries = json_decode(getURIData('http://places.labs.coop/v1/list/list/json.api', 20, 30), true);
...
if (!function_exists("getPlaceID")) {
	...
	function getPlaceID($countryid = '', $name = '', $countriessupported = array())
	{
		...
		if (!empty($count))
			$place = json_decode(getURIData('http://places.labs.coop/v1/$count/$name/json.api', 20, 30), true);
		else
			foreach($countries['countries'] as $country => $values)
			{
				$country = strtolower(str_replace(" ", "", $country));
				$place = json_decode(getURIData('http://places.labs.coop/v1/$country/$name/json.api', 20, 30), true);
...
if (!function_exists("getIPIdentity")) {
	...
	function getIPIdentity($ip = '', $sarray = false)
	{
		...
		if (!$row = $GLOBALS['trackerDB']->fetchArray($GLOBALS['trackerDB']->queryF($sql['selecta'])))
			if (($ipaddypart[0] ===  $serverpart[0] && $ipaddypart[1] ===  $serverpart[1]) )
			{
				...
				if (!$row = $GLOBALS['trackerDB']->fetchArray($GLOBALS['trackerDB']->queryF($sql['selectb'])))
				{
					$row = array();
					$row['ipaddy'] = $data['ip'];
					if (!filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) === false)
					...
					$GLOBALS['trackerDB']->queryF($sql['inserta']);
				} 
			} 
		$sql['updatea'] = "UPDATE `networking` SET `last` = '". time() . '\' WHERE `id` = "' . $row['id'] .'"';
		$GLOBALS['trackerDB']->queryF($sql['updatea']);
		if ($sarray == false)
			return $row['id'];
		else
			return $row;
